...
def home(sock,masterpass):
    ...
    def getpassword():
        ...
        key = masterpass[0:8]
        s = encryption.encryption("abcdefgh".encode().hex(),key.encode().hex())
        val = sock.recv(1024).decode()
    ...
    domain_name_dropdown.config(values=domain_names) #line was added here based on intuition
    ...
    b2 = Button(root, text="Back", font=myFont2, padx=30, bg=c3, command=lambda: destroy(root))
